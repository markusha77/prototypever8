...
const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  return (
    <div className="relative" ref={containerRef}>
      <div
        className={`border ${isOpen ? 'border-indigo-500 ring-2 ring-indigo-200' : 'border-gray-300'} rounded-lg bg-white p-1.5 cursor-text transition-all duration-200`}
        onClick={() => {
          setIsOpen(true);
          inputRef.current?.focus();
        }}
      >
        <div className="flex flex-wrap gap-1.5">
          {selectedValues.map(value => (
            <div
              key={value}
              className="flex items-center bg-indigo-50 text-indigo-700 text-sm rounded-md px-2 py-1"
            >
          ...
          <input
            ...
            className="flex-grow min-w-[120px] py-1 px-2 bg-transparent border-none focus:outline-none text-sm"
          />
        </div>
          ...
          {filteredOptions.length === 0 && searchTerm ? (
            ...
            <ul className="py-1">
              {filteredOptions.map(option => (
                ...
                >
                  <div className="w-5 h-5 mr-2 border border-gray-300 rounded flex items-center justify-center">
